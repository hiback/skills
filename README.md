...
- **[implement-batch](./skills/engineering/implement-batch/SKILL.md)** — Optional promoted serial batch executor that runs the same build as `implement` once per ticket; exported by this fork's plugin.
...
- **[implement-batch](./skills/engineering/implement-batch/SKILL.md)** — Optionally execute an approved spec or explicit ticket set as one recoverable serial batch, giving every ticket a fresh implementer, its own branch, a `/code-review` gate, and one squash commit.
...
- **[wait-what](./skills/productivity/wait-what/SKILL.md)** — Fire this the moment a message doesn't land. The agent re-pitches it with the context you're missing, in plain language, using your `CONTEXT.md` vocabulary.
