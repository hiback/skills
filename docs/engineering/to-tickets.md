...
Upstream is [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec to slice against — keep both in one unbroken context window. The default downstream is [implement](https://aihero.dev/skills-implement), which builds one ticket per fresh session, driving [tdd](https://aihero.dev/skills-tdd) for the tests and closing with [code-review](https://aihero.dev/skills-code-review). In this fork, [implement-spec-tickets](https://github.com/hiback/skills/blob/main/docs/engineering/implement-spec-tickets.md) is the optional serial controller when the approved frontier should run as one recoverable batch. When you're unsure which path fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
