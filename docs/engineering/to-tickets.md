...
`to-tickets` stops at the artifact; it never dispatches implementation itself. The default path is manual: look at the board, count the tickets with no open blockers, and open that many [implement](https://aihero.dev/skills-implement) sessions, one ticket per fresh context. This fork also ships [implement-batch](https://github.com/hiback/skills/blob/main/docs/engineering/implement-batch.md) as an explicit, separate command for serial batch execution of an approved spec or explicit ticket set with recovery state and review gates. In either path, keep the tracker's ticket state current because `implement` does not reliably close or check off a ticket when it finishes.
...
Upstream is [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec to slice against — keep both in one unbroken context window. The default downstream is [implement](https://aihero.dev/skills-implement), which builds one ticket per fresh session, driving [tdd](https://aihero.dev/skills-tdd) for the tests and closing with [code-review](https://aihero.dev/skills-code-review). In this fork, [implement-batch](https://github.com/hiback/skills/blob/main/docs/engineering/implement-batch.md) is the optional serial controller when an approved spec or explicit ticket frontier should run as one recoverable batch. When you're unsure which path fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
